...
- `/documents/{document_id}`：单据详情。等待状态显示本单原件/提取表单；发票存在所选收货时进入同一路由的核对布局，左右展示 Invoice 与 Receive Note 原件，多张收货以标签切换，下面显示逐行结果、备注和确认入口。收货单显示原件/字段及关联发票，没有“确认对账”按钮。
- `/history`：新工作台正式核对历史，默认显示当前门店全部不可变 Confirmation，包括重开前的旧快照；支持按发票号、供应商、收货单号搜索及按结果筛选。
- `/history/{confirmation_id}`：只读查看该次确认当时的关联编号、结果、未核验维度、差异说明、逐行快照、操作人与时间，并可导出该快照 CSV。
- `/history/legacy`：旧对账/Case 页面次级只读入口。旧 URL 仍可读取；编辑按钮隐藏，服务端旧业务写接口拒绝，不能靠隐藏按钮控制。
...
- 从上传入口打开的 Receive Note 在后台形成唯一关联后，用 replace 导航进入对应 Invoice 核对页；没有关系继续等待，多个关联只显示选择入口，不自动跳转。普通历史打开收货单不得强制跳转。
