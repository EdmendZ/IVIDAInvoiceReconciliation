...
## IR-SIMPLE-1.0.17 TapTouch 来源身份一致性

T21 完成后，补强同一 TapTouch 收货来源版本链的供应商 ID 一致性。外部 ID 只作为来源链防漂移的保护，不成为 Invoice 匹配字段；T22 只修改仓储决策、固定测试和产品说明，不扩展接口或数据结构。

## IR-SIMPLE-1.0.18 合成数据集工作台验收

用户确认继续使用本地已有测试数据集。新增 T23 将 Gold JSON 接入现有匹配与比对纯规则，PDF 仅做完整性和来源支持校验；缺失数据时明确跳过，不接入 TapTouch、真实 OCR/模型或新的业务模块。

## IR-SIMPLE-1.0.19 评测数据网页导入

用户要求在网页端查看这套数据。新增 T24 复用现有上传、Draft、Workspace Worker 和展示链路，提供非生产、幂等、带 fixture 标记的 17 文档导入入口；不把规则测试误认为数据库种子，也不扩展业务模块。
