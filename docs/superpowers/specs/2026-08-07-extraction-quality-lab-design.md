...
1. 合成 PDF 明确打印 Gold 所要求的供应商名称和商品行号；
2. 数据集校验能拒绝关键 Gold 字段缺少原文支持的样本；
3. 能创建两个配置不同且溯源完整的实验定义；
4. 能在同一数据集上运行 baseline 和 candidate；
5. 报告能定位到具体文档、字段、错误类型和业务场景；
6. 人工字段修改能够生成 Feedback Candidate；
7. 未经确认的反馈不能进入 Gold；
8. Promotion Decision 能解释每个门槛的通过或失败；
9. 全部 17 份合成文档运行时，单份失败不会使统计失真；
10. 真实 API 冒烟和缓存复用均有可验证记录；
11. 面试中能够在 5–8 分钟内演示真实实验、错误分析和版本决策；
12. 文档明确区分实验推荐与人工发布，不能声称已实现自动模型上线。
