...
工作台沿用确定性候选与比对规则：同 PO 的多个收货单先聚合，单位不明或主体冲突进入
blocked；价格缺失保持 `unverified`，覆盖范围显示为 `quantity_only`。匹配候选不会绕过
人工确认，已占用收货仍可在候选中解释但不能再次选择。预览采用来源代次和单据 revision
的 CAS 保存，旧计算不能覆盖新输入。
# 中文核对与差异处置界面

核对结果、候选评分说明、差异处理单、处置类型及审计操作标签现使用中文。系统规则提示在显示时翻译，英文商品数据和人工填写的说明不翻译；exact/within_tolerance 等结果枚举与 CSV 导出契约保持不变。语言调整不改变本篇核对算法、容差或审批规则。
